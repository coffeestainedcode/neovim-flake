...
in {
    ...
    config = {
        customNeovim.installedPlugins = [
            ...
            (if (cfg.onedark.enable)    then "onedark"    else "")
        ];
        ...
        in assert (assertionA && assertionB) || abort
        ...
        [{
            ...
            content = ''
                ${if cfg.catppuccin.setOnStartup then "vim.cmd.colorscheme 'catppuccin-${cfg.catppuccin.style}'" else ""}
                ${if cfg.tokyonight.setOnStartup then "vim.cmd.colorscheme 'tokyonight-${cfg.tokyonight.style}'" else ""}
                ${if cfg.gruvbox.setOnStartup then ''
                    vim.o.background = "${cfg.gruvbox.style}"
                    vim.cmd.colorscheme 'gruvbox'
                '' else ""}
                ${if cfg.onedark.setOnStartup then ''
